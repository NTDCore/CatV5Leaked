...
local function GetScript(scripturl)
	if shared.VapeDeveloper then
		...
		return game:HttpGet("https://raw.githubusercontent.com/NTDCore/CatV5Leaked/main/"..scripturl, true)
	end
end
...
runcode(function()
			...
			if callback then
				...
				RunLoops:BindToHeartbeat("Speed", 1, function(delta)
					if entity.isAlive and (GuiLibrary["ObjectsThatCanBeSaved"]["Lobby CheckToggle"]["Api"]["Enabled"] == false or matchState ~= 0) then
						...
						elseif speedmode["Value"] == "Heatseeker2" then
							if speedspeedup["Enabled"] and killauranear ~= lastnear then 
								if killauranear then 
									speedtick = tick() + 5
								else
	...
	speedmode = speed.CreateDropdown({
		["Name"] = "Mode",
		["List"] = {"Watchdog", "Normal", "CFrame", "Heatseeker","Heatseeker2"},
		["Function"] = function(val)
...
runcode(function()
	local clientcrasherreal = {["Enabled"] = false}
		clientcrasherreal = GuiLibrary["ObjectsThatCanBeSaved"]["CatV5Window"]["Api"].CreateOptionsButton({
			["Name"] = "ClientCrasher",
			["Function"] = function(callback)
				if callback then
					game:GetService("NetworkClient"):SetOutgoingKBPSLimit(math.huge)
...
if shared.nobolineupdate then
	...
	task.spawn(function()
		...
		local window = kavo.CreateLib("Noboline v1.7.3"..(shared.VapePrivate and " - PRIVATE" or ""), "Ocean")
...
runcode(function()
	...
	BetterHighJump = GuiLibrary["ObjectsThatCanBeSaved"]["CatV5Window"]["Api"].CreateOptionsButton({
		["Name"] = "BetterHighJump",
		["Function"] = function(callback)
			if callback then
...
runcode(function()
	...
	local Step = 0
	TPFly = GuiLibrary["ObjectsThatCanBeSaved"]["CatV5Window"]["Api"].CreateOptionsButton({
		["Name"] = "TPFly",
		["Function"] = function(callback)
			if callback then
